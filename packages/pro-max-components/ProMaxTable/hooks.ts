...
import type { ProMaxTableRef } from './index';
...
export interface ProMaxTableState {
  ...
  optionsSetStoreRef: React.MutableRefObject<{
...
export const useProMaxTableState = () => {
  ...
  // 用于设置表单的下拉选项
  const optionsSetStoreRef = React.useRef<{
    [key: string]: (options: any[]) => void;
  }>({});
...
export const useOptionsSetStoreRef = () => {
  const { optionsSetStoreRef } = React.useContext(ProMaxContext) || {};
